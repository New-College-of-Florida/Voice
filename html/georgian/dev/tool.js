...
function change_y_axis_cents(){
    var update = {
        'yaxis.side': 'right',
        'yaxis.overlaying': 'y2',
        'yaxis2.side': 'left',
        'yaxis2.overlaying': null
    }
    Plotly.relayout(plot, update)
}
function change_y_axis_frequency(){
    var update = {
        'yaxis.side': 'left',
        'yaxis.overlaying': null,
        'yaxis2.side': 'right',
        'yaxis2.overlaying': 'y'
    }
    Plotly.relayout(plot, update)
...
async function get_voice(collectionName, songName, voiceName) {
  ...
  dataYcents = [];
  for(var i = 0;i<dataY.length;i++) {
    dataYcents.push(parseFloat(1200 * Math.log2(dataY[i]/55)));
  }
  console.log(dataYcents);
  
  return [dataX, dataY, data_mad_low, data_mad_high, dataMad, dataYcents];
...
async function update_plot(collectionName, songName, voiceName) {
  ...
  var bass_trace_cents = {
    x: bass_data[0],
    y: bass_data[5],
    mode: 'markers',
    name: 'Bass voice cents',
    type: 'scattergl',
    marker: {
      color: 'red',
      size: 3,
...
//-----------------------------------
  ...
  var data = [bass_trace, bass_trace_cents, bass_mad_high_trace, bass_mad_low_trace, mid_trace, mid_trace_cents, mid_mad_low_trace, mid_mad_high_trace, top_trace, top_trace_cents, top_mad_low_trace, top_mad_high_trace];
